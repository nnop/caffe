...
namespace caffe {
  ...
  template <typename Dtype>
  void PSROIPoolingForwardCPU (
    const int count,
    const Dtype* const bottom_data_ori,
    const Dtype spatial_scale,
    const int channels,
    const int height, const int width,
    const int pooled_height, const int pooled_width,
    const Dtype* const bottom_rois_roi,
    ...
    for (int index = 0; index < count; ++index) {
      ...
      const Dtype* bottom_rois = bottom_rois_roi + n * 5;
      ...
      const Dtype* bottom_data = bottom_data_ori + (roi_batch_ind * channels + c) * height * width;
  ...
  void PSROIPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    PSROIPoolingForwardCPU(count, bottom_data, spatial_scale_,
